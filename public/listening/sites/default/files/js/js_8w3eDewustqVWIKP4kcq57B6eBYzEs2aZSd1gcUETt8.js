(function ($,Drupal,drupalSettings) {
         //dragDrop changes
         var drag_val = localStorage.getItem("dragOptions");   
         localStorage.removeItem("dragOptions"); 
         if(drag_val != ''){
             var obj = jQuery.parseJSON( drag_val );
             $.each(obj,function(key,val){
                 if(val){
                    var newArr = val.split('!!!'); 
                    $("#"+newArr[0]).removeClass('demo').text(newArr[1]);
                 }                  
             });
...
//        localStorage.removeItem('row_id');
        ...
        $(".drag_drop").each(function(index){            
            ...
            if(abc != undefined) {                
                drag_option[index] = $(this).attr("id")+'!!!'+abc;                
            }
        }); 
        localStorage.setItem('dragOptions', JSON.stringify(drag_option));
...
//        alert(data);return false;
    ...
    function getData() {
        ...
        type_drop.each(function (){
            ...
            //dragDrop changes
            var dropInputChoice = $(this).find('td[class="answer-wp"]');            
            dropInputChoice.each(function (){                
                var num = $(this).find('span').attr('rel');
                var type = $(this).find('span').attr('data-q_type');
                var qu_id = $(this).find('span').attr('rel1');
                var ans_selected = $(this).find('span').text();
                if(ans_selected != ''){
                    var ans = ans_selected;
                }else {
                    var ans = '';
                    if($("#tag-choice-"+qu_id).find("div[rel='"+num+"']").hasClass('inside')){
                        var ans = $("#tag-choice-"+qu_id).find("div[rel='"+num+"']").text();
                    }
                }                
                data.answers[num] = {'num':num,'type':type,'ans':ans};
            });
